...
//TODO: - Controls
    ...
    @IBOutlet weak var lblNoFollowers: UILabel!
...
//TODO: - Let's Code
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
         ...
         loginID  = General.loadSaved("user_id")
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell{
       ...
       // cell.imgDogTags.image = UIImage(named: "img_dogtagBG-small\(self.delObj.deviceName)")
        ...
        if(loginID == data.userID){
            cell.btnStatus.hidden = true
        }else{
            cell.btnStatus.hidden = false
        }
        ...
        if(data.is_following == "1"){
             cell.btnStatus.setTitle("Unfollow", forState: UIControlState.Normal)
        }else{
            if(data.is_requested == "1"){
                cell.btnStatus.setTitle("Requested", forState: UIControlState.Normal)
            }else{
                cell.btnStatus.setTitle("Follow", forState: UIControlState.Normal)
            }
            
        }
       
        cell.btnStatus.addTarget(self, action: #selector(FollowersViewController.btnStatusClick(_:)), forControlEvents: UIControlEvents.TouchUpInside)
...
//TODO: - UIButton Action
    ...
    @IBAction func btnOtherUserProfileClick(sender:UIButton){
        ...
        if(requestDetails.count>0){
            ...
            if(userid != followID){
                ...
                //self.navigationController?.presentViewController(otherVC, animated: true, completion: nil)
            }else{
    ...
    @IBAction func btnStatusClick(sender:UIButton){
        //MARK: Check account is verified or not
        let veriStatus = General.loadSaved("verification_pending")
        if(veriStatus == "1"){
            ...
            if(data.is_following == "1"){
                ...
                //MARK: Follow User
                 self.followUser(String(data.userID), button: sender)
            }
...
//TODO: - Web service / API Implementation
    ...
    /**
     Fetch followers for login user
     
     - parameter params:
     */
    func fetchFollowerList(params: [String: AnyObject]) {
        
        self.view.userInteractionEnabled = false
        print(params)
        self.cust.showLoadingCircle()
        Alamofire.upload(.POST, Urls_UI.LIST_FOLLOWERS, multipartFormData: {
                                    ...
                                    if status != "0" {
                                        ...
                                        if(count>0){
                                            for ind in 0...count!-1{
                                                ...
                                                let is_following = json["data"][ind]["is_following"].stringValue
                                                let is_requested = json["data"][ind]["is_requested"].stringValue
                                                self.requestDetails.append(FollowerRequestModel(dogtagid: dogtagid, name: user_fullname, location: city_state, country: country, status: branch_name, profile: user_profile_pic,userID: user_id,abbreviation: abbreviation,is_following:is_following,is_requested:is_requested)!)
                                            ...
                                            self.lblNoFollowers.hidden = false
                                        }
                                    }
                                }
