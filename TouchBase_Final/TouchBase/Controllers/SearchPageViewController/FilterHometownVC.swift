...
class FilterHometownVC: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        let dataList = General.loadSaved("search_home_city").characters.split{$0 == ","}.map(String.init)
        for row in dataList {
            selectedRow.append(row)
        }
        htCountry = General.loadSaved("search_home_country")
        htState = General.loadSaved("search_home_state")
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell{
        ...
        let city = self.citiesHome[indexPath.row]
    ...
    @IBAction func saveClicked(sender: UIButton) {
        General.saveData(htCountry, name: "search_home_country")
        General.saveData(htState, name: "search_home_state")
        ...
        General.saveData(values, name: "search_home_city")
